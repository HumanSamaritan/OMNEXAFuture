...
export async function POST(request: Request) {
  try {
    ...
    if (ownerResult.error) {
      console.error("Resend owner email error", { error: getErrorMessage(ownerResult.error) });
      return NextResponse.json(
    ...
    if (sendConfirmationEmail) {
      ...
      if (confirmationResult.error) {
        console.error("Resend confirmation email error", { error: getErrorMessage(confirmationResult.error) });
      }
    }
